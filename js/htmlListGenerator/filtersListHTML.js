...
const filtersListHTML = () => {
	...
	for (const category in filtersData) {
		...
		if (category === 'Size') {
			filtersCategoryList.classList.add(
				'filters__category-list',
				'filters__category-list--size',
				'filters__category-list--is-open'
			);
		} else {
			filtersCategoryList.classList.add(
				'filters__category-list',
		...
		filtersCategoryList.dataset.filtersListValue =
		...
		filtersData[category].forEach(item => {
			...
			if (item === 'all') {
				filtersCategoryListItem.classList.add(
					'filters__category-list-item',
					'filters__category-list-item--color-is-checked'
				);
			} else {
				filtersCategoryListItem.classList.add('filters__category-list-item');
			}
			filtersCategoryListItem.dataset.filterValue = item;
			filtersCategoryListItem.textContent =
		...
		if (category === 'Color') {
			filtersCategoryList.innerHTML =
				`<button class="filters__category-list--colors-confirm" data-filter-value="confirmColor">Ok</button>` +
				filtersCategoryList.innerHTML;
		}
